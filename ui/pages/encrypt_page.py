...
class EncryptPage(ctk.CTkFrame):
    def __init__(self, parent, accent="#1fb6a8"):
        ...
        title = ctk.CTkLabel(self, text="Cifrar / Descifrar", font=HEADING_FONT)
        ...
        # tarjetas para los controles
        frame = CardFrame(self, fg_color="#121213", corner_radius=14, border_color="#1b1c1f")
        frame.pack(padx=30, pady=12, fill="x")

        btn_style = button_kwargs(self.accent)
        btn_load_pub = ctk.CTkButton(frame, text="📂 Cargar clave pública (para cifrar)", command=self.load_public_key, **btn_style)
        btn_load_pub.pack(side="left", padx=12, pady=12)

        btn_load_priv = ctk.CTkButton(frame, text="📂 Cargar clave privada (para descifrar)", command=self.load_private_key, **btn_style)
        btn_load_priv.pack(side="left", padx=12, pady=12)

        ops = CardFrame(self, fg_color="#121213", corner_radius=14, border_color="#1b1c1f")
        ops.pack(padx=30, pady=12, fill="x")

        btn_encrypt = ctk.CTkButton(ops, text="🔒 Cifrar un archivo .txt", command=self.encrypt_file, **btn_style)
        btn_encrypt.pack(side="left", padx=12, pady=12)

        btn_decrypt = ctk.CTkButton(ops, text="🔓 Descifrar un archivo .enc", command=self.decrypt_file, **btn_style)
        ...
        # caja central con fuente mucho más grande (3x)
        style_central_textbox(self.output)
    ...
    def encrypt_file(self):
        if not self._pub_path:
            show_error(self, "Error", "Primero carga una clave pública.")
            return
        ...
        try:
            ...
            self.output.see("end")
            show_info(self, "Cifrado completo", f"El archivo fue cifrado con éxito:\n{out}")
        except Exception as e:
            show_error(self, "Error", f"Error al cifrar el archivo:\n{e}")

    def decrypt_file(self):
        if not self._priv_path:
            show_error(self, "Error", "Primero carga una clave privada.")
            return
        ...
        try:
            ...
            self.output.see("end")
            show_info(self, "Descifrado completo", f"El archivo fue descifrado con éxito:\n{out}")
        except Exception as e:
            show_error(self, "Error", f"Error al descifrar el archivo:\n{e}")

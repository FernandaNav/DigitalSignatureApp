...
from ui.styles import show_error, show_info, show_warning, style_textbox, HEADING_FONT, SUBHEADING_FONT, BUTTON_FONT, button_kwargs, style_central_textbox
...
class SignPage(ctk.CTkFrame):
    def __init__(self, parent, accent="#1fb6a8"):
        ...
        title = ctk.CTkLabel(self, text="Firmar / Verificar", font=HEADING_FONT)
        ...
        btn_sign = ctk.CTkButton(frame, text="🖋 Firmar un archivo .txt", command=self.sign_file, **btn_style)
        btn_sign.pack(side="left", padx=12, pady=12)

        btn_verify = ctk.CTkButton(frame, text="🔎 Verificar archivo + firma", command=self.verify_file, **btn_style)
        ...
        # caja central con letra más grande
        style_central_textbox(self.log)
    ...
    def sign_file(self):
        ...
        try:
            ...
            self.log.see("end")
            show_info(self, "Archivo firmado", f"Se generó la firma correctamente:\n{out}")
        except Exception as e:
            show_error(self, "Error", f"No se pudo firmar el archivo:\n{e}")
    ...
    def verify_file(self):
        ...
        try:
            ...
            if ok:
                ...
                show_info(self, "Verificación exitosa", "La firma es válida y el archivo no fue modificado.")
            else:
                self.log.insert("end", "❌ Firma inválida\n")
                show_warning(self, "Firma inválida", "La firma no coincide o el archivo fue alterado.")
            self.log.see("end")
        except Exception as e:
            show_error(self, "Error", f"No se pudo verificar la firma:\n{e}")
